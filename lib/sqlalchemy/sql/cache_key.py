...
class _CacheKeyTraversal(
    ...
    # custom traversal subclasses may still use Python methods, which are
    # dispatched under the fallback _PY_METHOD kind
    ...
    visit_compile_state_funcs = InternalTraversal.dp_compile_state_funcs
    visit_inspectable = InternalTraversal.dp_inspectable
    visit_string_list = InternalTraversal.dp_string_list
    visit_multi = InternalTraversal.dp_multi
    visit_multi_list = InternalTraversal.dp_multi_list
    visit_has_cache_key_tuples = InternalTraversal.dp_has_cache_key_tuples
    visit_has_cache_key_list = InternalTraversal.dp_has_cache_key_list
    visit_executable_options = InternalTraversal.dp_executable_options
    visit_inspectable_list = InternalTraversal.dp_inspectable_list
    visit_clauseelement_tuples = InternalTraversal.dp_clauseelement_tuples
    visit_fromclause_ordered_set = InternalTraversal.dp_fromclause_ordered_set
    # visit_clauseelement_unordered_set is obsolete; InternalTraversal has no
    # corresponding dispatch symbol.
    visit_named_ddl_element = InternalTraversal.dp_named_ddl_element
    visit_prefix_sequence = InternalTraversal.dp_prefix_sequence
    visit_setup_join_tuple = InternalTraversal.dp_setup_join_tuple
    visit_table_hint_list = InternalTraversal.dp_table_hint_list
    visit_plain_dict = InternalTraversal.dp_plain_dict
    visit_dialect_options = InternalTraversal.dp_dialect_options
    visit_string_clauseelement_dict = (
        InternalTraversal.dp_string_clauseelement_dict
    )
    visit_string_multi_dict = InternalTraversal.dp_string_multi_dict
    visit_fromclause_canonical_column_collection = (
        InternalTraversal.dp_fromclause_canonical_column_collection
    )
    visit_unknown_structure = InternalTraversal.dp_unknown_structure
    visit_dml_ordered_values = InternalTraversal.dp_dml_ordered_values
    visit_dml_values = InternalTraversal.dp_dml_values
    visit_dml_multi_values = InternalTraversal.dp_dml_multi_values
    visit_params = InternalTraversal.dp_params
